...
export function POSCard({ product, gravityEnabled, onAddToCart }: POSCardProps) {
    ...
    const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        if (!gravityEnabled || !cardRef.current) return
    ...
    return (
        <motion.div
            ...
            style={{
                rotateX: gravityEnabled ? rotateX : 0,
                rotateY: gravityEnabled ? rotateY : 0,
                transformStyle: "preserve-3d",
            }}
            className="glass-strong rounded-[2.5rem] p-5 relative overflow-hidden group transition-all duration-500 hover:shadow-2xl hover:shadow-primary/10 border border-white/10"
        >
            {/* Glossy Overlay */}
            <div className="absolute inset-0 bg-gradient-to-tr from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />

            {/* Product Image Placeholder/Icon */}
            <div className="relative h-48 mb-4 rounded-3xl overflow-hidden bg-gradient-to-br from-primary/5 to-secondary/5 flex items-center justify-center">
                {product.images?.[0] ? (
                    <img
                    ...
                    <Package className="w-12 h-12 text-primary/20 group-hover:scale-110 transition-transform duration-500" />
                )}
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center justify-between pt-2">
                    ...
                    <Button
                        ...
                        className="rounded-2xl h-12 w-12 bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all active:scale-95 group/btn overflow-hidden relative"
                    >
                        <motion.div
                            whileHover={{ y: "-50%" }}
            ...
            {/* Float effect elements if gravity is on */}
            {gravityEnabled && (
