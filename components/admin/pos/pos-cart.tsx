        ...
        <div className="flex flex-col h-full glass-strong rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl">
                ...
                <AnimatePresence mode="popLayout">
                    {items.map((item) => (
                        <motion.div
                            ...
                            className="glass-light rounded-2xl p-3 flex items-center gap-3 group relative border border-white/5 hover:border-white/20 transition-colors"
                        >
                            <div className="w-16 h-16 rounded-xl bg-primary/5 flex items-center justify-center overflow-hidden shrink-0">
                                {item.image ? (
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {item.variant_name && (
                                    <p className="text-[10px] text-primary font-bold uppercase tracking-wider">{item.variant_name}</p>
                                )}
                                <p className="text-xs text-muted-foreground font-medium">{item.price} MAD</p>
            ...
            <div className="p-6 bg-white/5 border-t border-white/5 space-y-4">
                ...
                <Button
                    ...
                    onClick={onCheckout}
                    className="w-full h-14 rounded-2xl bg-primary hover:bg-primary/90 text-lg font-bold shadow-xl shadow-primary/20 group relative overflow-hidden"
                >
                    <motion.div
